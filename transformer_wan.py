...
from dispatch import dispatch_attention_fn
...
class WanAttnProcessor2_0:
    ...
    def __call__(
        ...
        # Convert tensors to appropriate format based on attention backend
        if self._attention_backend == "sage":
        ...
        if attn.add_q_proj is not None and attn.add_k_proj is not None and attn.add_v_proj is not None:
            encoder_hidden_states_img = encoder_hidden_states
            query_img = attn.add_q_proj(encoder_hidden_states_img)
            key_img = attn.add_k_proj(encoder_hidden_states_img)
            key_img = attn.norm_added_k(key_img)
            value_img = attn.add_v_proj(encoder_hidden_states_img)

            # Convert image embeddings to appropriate format
            if self._attention_backend == "sage":
                query_img, key_img, value_img = ensure_hnd_format(query_img, key_img, value_img)
            else:
                query_img, key_img, value_img = ensure_nhd_format(query_img, key_img, value_img)
